...
from torchtitan.components.optimizer.muon import MuonWithAuxAdam
...
def build_optimizers(
    ...
    **Note**
    ...
    # Muon optimizer will be handled in the normal path below
    ...
    if name in ["Adam", "AdamW"]:
        ...
        # Get MUON-specific config values
        muon_lr = getattr(optimizer_config, "muon_lr", 0.02)
        muon_momentum = getattr(optimizer_config, "muon_momentum", 0.95)
        muon_wd = getattr(optimizer_config, "muon_wd", 0.0)
        muon_nesterov = getattr(optimizer_config, "muon_nesterov", True)
        muon_ns_steps = getattr(optimizer_config, "muon_ns_steps", 3)
        
        adamw_lr = getattr(optimizer_config, "adamw_lr", 3e-4)
        adamw_betas = (beta1, beta2)
        adamw_eps = eps
        adamw_wd = getattr(optimizer_config, "adamw_wd", 0.0)
        
        logger.info(f"MUON config: muon_lr={muon_lr}, adamw_lr={adamw_lr}")
        logger.info(f"MUON config: muon_wd={muon_wd}, adamw_wd={adamw_wd}")
        ...
        raise NotImplementedError(f"Optimizer argument handling for {name} not implemented.")
